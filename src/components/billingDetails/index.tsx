...
const BillingDetails: React.FC<BillingDetailsProps> = (props) => {
  ...
  const billDeatail = useMemo(
    ...
    [user?.billNo, invoice?.invoiceDate, invoice?.receiptNo, user?.age, user?.name]
  );

  const totalServiceAmount = invoice?.services.reduce((accumulator, service) => {
    const serviceTotal = service.price * service.quantity;
    return accumulator + serviceTotal;
  }, 0);

  const totalPrice = useCallback((totalServiceAmount: number, tax: number, discount: number) => {
    return totalServiceAmount + tax - discount;
  }, []);

  useEffect(() => {
    fetchData();
    const total = totalPrice(totalServiceAmount ?? 0, invoice?.tax ?? 0, invoice?.disCount ?? 0);
    setTotal(total);
  }, [fetchData, user?.billNo, invoice?.disCount, invoice?.tax, totalPrice, totalServiceAmount]);
          ...
          {invoice &&
            invoice?.services.map((service: Service, i: number) => (
              <Box
                ...
                <Typography>{service && totalAmount(service?.quantity, service?.price)}</Typography>
              </Box>
            ))}
        </Box>
